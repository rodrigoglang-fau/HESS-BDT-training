...
def makeLookups(task, workDirectory, config, environmentVariables, muonPhase="", thetaSqr=0, zetaBDT=0):
...
def makeLookups_OffEnergyShape(workDirectory, config, zenithAngles, muonPhase, environmentVariables):
    ...
    for zenith in zenithAngles:
        ...
        command += ' --config ' + config + ' --Diagnostics/DiagnosticFolder Preselect --Diagnostics/ListOfVariables HillasImageAmplitude,HillasWidth,HillasLength,ImpactParameter,CameraXEvent,CameraYEvent'
