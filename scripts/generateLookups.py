...
def makeLookups_OffEnergyShape(workDirectory, config, zenithAngles, muonPhase, environmentVariables):
    ...
    for zenith in zenithAngles:
        ...
        #command += '{}/hddst/scripts/hap_split_mc.pl --include {} --filelist {} --outfile {}'

        command += ' --outdir {}/hap'.format(environmentVariables["HESSROOT"], workDirectory + '/temp_scripts/empty.conf', listName, output, workDirectory)
    ...
    if ('hybrid' in config) or ('stereo' in config):
        isHybrid = 'true' # this is needed to generate lookups on the HESSIILookups style
