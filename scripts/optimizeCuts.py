...
def runOptimization(workDirectory, config, muonPhase, zenith, azimuth, offset, spectralIndex, normalizationFactor, minimumSignalEfficiency, verbose, environmentVariables):
    ...
    for fname in f_off:
        ...
        try:
            ...
            print("WARNING! Problems reading the trees in the offruns file " + fname + ", please check that these variables are present: 'RunNr', 'RaSystem', 'DecSystem', 'RaEvent', 'DecEvent', 'CorrEnergy', " + opt_var + "! If just a few files are corrupt, the optimization process should work fine.")
    ...
    #if 'mono' in config:
    #    zeta_scan_bins = np.linspace(-0.95, -0.2, 131)
    #    if config == 'ultraloose_zeta_mono':
